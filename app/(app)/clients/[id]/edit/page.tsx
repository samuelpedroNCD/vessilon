...
export default async function EditClientPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [client, brokers] = await Promise.all([getClient(supabase, id), brokerOptions(supabase)]);
  if (!client) notFound();
  return (
    <AppShell active="buyers" user={shellUser(profile)}>
      ...
      <ClientForm action={updateClientRecord.bind(null, id)} client={client} brokers={brokers} submitLabel="Save changes" />
    </AppShell>
  );
}
