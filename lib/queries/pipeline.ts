...
export async function getOpportunity(supabase: SupabaseClient, id: string) {
  const { data } = await supabase
    .from("opportunities")
    .select("*, stage:lob_stages(id, name, position, probability, is_won, is_lost), yacht:yachts(id, name, hero_color, builder, loa_m, hull_id, price), client:clients(id, name), lead:leads(id, name), broker:profiles!assigned_broker(id, full_name)")
    .eq("id", id)
    .single();
